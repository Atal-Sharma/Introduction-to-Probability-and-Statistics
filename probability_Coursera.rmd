...
```{r, echo=FALSE}
print("E. The shortest streak is of length 1")
```
## Compared to What?
...
print("Typical Streak Length = 0")
print("Longest Streak = 3")
...
ggplot(data = sim_streak2 , aes(x = length)) + geom_histogram(binwidth = 1)
print("B. Somewhat Similar")
```
...
```{r}
print("A. The distributions look very similar. Therefore, there doesn't appear to be evidence for Kobe Bryant's hot hand.")
```
...
```{r, echo=FALSE}
print("The topics covered in this lab assignment are Random Sampling, Hypothesis test(by initially defining a nul hypothesis and checking it against the p-value. Few othere topics that could've been covered are Probability - joint, posterior, condional and marginal. An additional assignment of simulation of Bayesian Inference would've been interesting too.")
```
</div>

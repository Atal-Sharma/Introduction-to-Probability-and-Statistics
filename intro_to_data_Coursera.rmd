...
ggplot(data = sfo_feb_flights , aes(x=sfo_feb_flights$arr_delay)) + geom_histogram()

sfo_feb_flights %>% summarize(mean_ad = mean(arr_delay) , sd_ad = sd(arr_delay) , n = n())
...
sfo_feb_flights %>% group_by(carrier) %>% summarize(median_ad = median(arr_delay) , IQR_ad = IQR(arr_delay) , n=n()) %>% arrange(desc(IQR_ad))
print("D")
...
nycflights <- nycflights %>% mutate(avg_speed = (60*distance)/air_time) %>% arrange(desc(avg_speed))
...
`del_type` with levels `"on time"` and `"delayed"` depending on whether there was any 
...
nycflights <- nycflights %>% mutate(del_type = ifelse(dep_delay == 0 , "on time" , "delayed"))
del_ot <- nycflights %>% filter(del_type == "delayed" , arr_type == "on time")
del_ot_percent = (100*NROW(del_ot))/NROW(nycflights)
del_ot_percent
```